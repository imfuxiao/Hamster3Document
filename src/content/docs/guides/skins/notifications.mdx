...
当通知中的内容与 rimeOptionName 和 rimeOptionValue 设置的内容匹配时，触发样式变化。当 rimeOptionName 匹配，而 rimeOptionValue 设的值与当前的值不匹配时，按键会恢复为原始样式。
...
# rimeSchemaID 与 rimeSchemaName 二选一设置，建议使用 rimeSchemaID，中文在不同编码下判断可能会有问题
rimeSchemaID: 方案ID
rimeSchemaName: 方案名称
`;
...
:::tip[注意]
- rimeSchemaID 与 rimeSchemaName 只需要配置其中任意一个参数即可，建议使用 rimeSchemaID，因为中文在不同编码下可能会有问题。
- 元书版本要求 1.6.10 及以上。或 TF 313 及以上版本。
:::
...
RIME 预编辑文本通知类型用于监听 RIME 预编辑文本的变化。

export const rimePreeditNotificationCode = `
...
<Code lang="yaml"
  code={rimePreeditNotificationCode}
  />

> 注意：这里设置的样式为 rime preedit 存在时的样式，当 preedit 为空时，按键会恢复为原始样式。

## 根据通知调整按键样式及动作

在通知设置中，可以通过 `backgroundStyle`，`foregroundStyle`，`action`, `swipeUpAction`, `swipeDownAction` 参数改变按键的样式及触发动作。
...
如果没有为通知设置按键动作 `action`, `swipeUpAction`, `swipeDownAction` 参数，则按键会继续使用原始样式中设置的 `action`, `swipeUpAction`, `swipeDownAction`。
</Aside>

### 锁定通知结果

可以使用参数 `lockedNotificationMatchState` 来锁定匹配后的状态。

注意：锁定状态后，当通知结果不匹配后，按键也不会发生。

示例：

```yaml
changeFlypyRimeSchemaNotification:
  notificationType: keyboardAction
  notificationKeyboardAction: { switchRimeSchema: 'aaa' }
  lockedNotificationMatchState: true
  foregroundStyle:
    # - rightArrowButtonForegroundStyle
    - rightArrowButtonSwitchChineseForegroundStyle
  action: { switchRimeSchema: 'bbb' }
```
