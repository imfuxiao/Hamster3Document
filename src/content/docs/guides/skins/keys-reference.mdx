---
title: 配置项索引（全部 Key）
description: 元书键盘皮肤 YAML 文件中全部可用 Key、值类型与枚举值速查表
---
...
<Aside type="tip" title="关于 YAML 合并键 `<<`">
...
| `moveStep` | `{ horizontal: Float, vertical: Float }` | 单元格尺寸 + `spacing` | 相对位移选择的灵敏度：手指移动多少距离切换一个单元格，值越小越灵敏。`horizontal` 与 `vertical` 必须同时配置才生效，最小值为 `1`。 |
| `moveThreshold` | `Float` | `10` | 相对位移选择的最小移动距离；手指位移未超过该值时维持初始状态（`selected` 单元格或无高亮）。 |
| `backgroundStyle` | `String` | — | 面板背景样式名。 |
...
<Aside type="note" title="`text` 的两个换行写法">
