...
- `t9Symbols`: 中文九键（T9）键盘纵向符号列表，即 T9 键盘中左侧纵向显示的列表。可以在「键盘设置」-「中文九键符号设定」中配置数据源。(v1.16.0 版本及以上支持)
- `t9HorizontalSymbols`: 中文九键（T9）横向符号列表，即水平显示一个 T9 拼音列表。可以在「键盘设置」-「中文九键符号设定」中配置数据源。（实验功能，需 TF >= 350）
...
- `interCellSpacing`: 可选，表示候选字之间的间距，默认值为 4
  > 注意：需 TF >= 463，商店版本 >= 1.6.28
...
- `backgroundCornerRadius`：可选，表示候选字背景圆角。
