...
- `t9Symbols`：中文九键（T9）键盘符号列表，即 T9 键盘中左侧纵向显示的列表。可以在「键盘设置」-「中文九键符号设定」中配置数据源。(v1.16.0 版本及以上支持)
- `t9HorizontalSymbols`：中文九键（T9）横向符号列表，用于横向显示 T9 符号数据。
...
- `backgroundStyle`：可选，引用集合视图背景样式。
- `cellStyle`：必选，引用集合视图中单元格的样式节点。
- `dataSource`：必选，引用数据源节点。

<Aside type="danger" title="注意">
  `cellStyle` 中的 `foregroundStyle` 样式只能配置字符串样式引用，且只能使用
  `text` 类型。
</Aside>
...
- `separatorLineColor`：可选，表示分割线颜色，默认值为系统默认分割线颜色。
...
- `type`：必选，表示集合视图类型。横向符号列表使用 `horizontalSymbols`，T9 横向符号列表使用 `t9HorizontalSymbols`。
- `size`：必选，表示集合视图尺寸。与键盘按键尺寸定义相同。
- `maxColumns`：可选，表示集合视图最大列数，默认值为 `5`。
  > 注意：只有当数据源中数据超过此数值后，列表才能滑动，少于或等于此数值时，视图无法滑动。
- `maxRows`：可选，表示集合视图最大行数。部分横向集合会读取此参数。
...
- `selectedStyle`：可选，引用选中状态下的单元格样式节点。
- `dataSource`：必选，引用集合视图的数据源节点。数据源是一个数组，数组中的每一项表示集合视图中的一个单元格。
...
- `maxRows`：可选，表示集合视图最大行数，默认值为 `5`。
- `separatorColor`：可选，表示候选字之间的分割线颜色。
- `backgroundStyle`：可选，引用集合视图背景样式。
- `candidateStyle`：必选，引用单个候选字的样式节点。
...
<Aside type="danger" title="注意">
  - `styleName` 属性必须与 `label` 同时使用 - `styleName` 引用的样式中，只能定义
  `backgroundStyle` 和 `foregroundStyle` 两个属性，其余属性无效
</Aside>
